...
data class CharacterBlueprintDefinition(
    ...
    @JsonProperty("stances") val stances: StancesDefinition,
    ...
    @JsonProperty("loot_table") val lootTable: LootTableDefinition,
    ...
    fun toCharacterBlueprint(
        ...
        hurtSound?.toSound(),
        deathSound?.toSound(),
        interactions.map { it.toInteraction(questRegistry) },
        lootTable.toLootTable(),
        scriptReference.resolve(scriptRegistry),
        secondsToMillis(removalDelaySeconds),
        secondsToMillis(respawnTimeSeconds)
    )
}
